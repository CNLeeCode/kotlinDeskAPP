...
object DatabaseManager {
    private const val DB_NAME = "ConnectionInfo.db"
    ...
    val database: AppDatabase by lazy (LazyThreadSafetyMode.SYNCHRONIZED) {
        ...
        try {
            if (!dbFile.exists() || dbFile.length() == 0L) {
                ...
                migrateSchema(driver, oldVersion = 1, newVersion = DB_VERSION)
            }
        }catch (err: Throwable) {
    ...
    private fun migrateSchema(driver: SqlDriver, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            driver.execute(
                null,
